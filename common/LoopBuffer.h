...
class LoopBuffer {
public:
    LoopBuffer(std::shared_ptr<CMemoryPool>& pool);
    ~LoopBuffer();
    ...
    // list point
    LoopBuffer* GetNext();
    void SetNext(LoopBuffer* next);

    friend std::ostream& operator<< (std::ostream &out, const LoopBuffer &obj);
...
private:
    ...
    std::mutex   _mutex;
    LoopBuffer* _next;         //point to next node
    std::shared_ptr<CMemoryPool>    _pool;
};

std::ostream & operator<< (std::ostream &out, const LoopBuffer &obj);
