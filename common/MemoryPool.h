...
public:
    ...
    MemoryPool(const int large_sz, const int add_num);
    ~MemoryPool();
...
template<typename T, typename... Args>
T* MemoryPool::PoolNew(Args&&... args) {
    int sz = sizeof(T);
    if (sz > __max_bytes) {
...
template<typename T>
void MemoryPool::PoolDelete(T* &c) {
    if (!c) {
        return;
    }
...
template<typename T>
T* MemoryPool::PoolMalloc(int sz) {
    if (sz > __max_bytes) {
        void* bytes = malloc(sz);
        memset(bytes, 0, sz);
        return (T*)bytes;
...
template<typename T>
void MemoryPool::PoolFree(T* &m, int len) {
    if (!m) {
        return;
    }
...
template<typename T>
T* MemoryPool::PoolLargeMalloc() {
    T* ret = (T*)_block_pool.PoolLargeMalloc();
    return ret;
}

template<typename T>
void MemoryPool::PoolLargeFree(T* &m) {
    if (!m) {
        return;
    }
