...
class Log: public RunnableAloneTaskList<char*>, public Single<Log> {
public:
    Log();
    virtual ~Log();
    virtual void Run();
...
#define LOG_DEBUG(log, ...)            Log::Instance().LogDebug(__FILE__, __LINE__, log, ##__VA_ARGS__);
#define LOG_INFO(log, ...)             Log::Instance().LogInfo(__FILE__, __LINE__, log, ##__VA_ARGS__);
#define LOG_WARN(log, ...)             Log::Instance().LogWarn(__FILE__, __LINE__, log, ##__VA_ARGS__);
#define LOG_ERROR(log, ...)            Log::Instance().LogError(__FILE__, __LINE__, log, ##__VA_ARGS__);
#define LOG_FATAL(log, ...)            Log::Instance().LogFatal(__FILE__, __LINE__, log, ##__VA_ARGS__);
