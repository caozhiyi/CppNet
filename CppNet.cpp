...
void ReadFunc(CMemSharePtr<CEventHandler>& event, int error) {
	...
	if (error != EVENT_ERROR_CLOSED) {
		event->_client_socket->SyncWrite("aaaaa21231231", strlen("aaaaa21231231"), write_back);
		event->_client_socket->SyncRead(read_back);
	} else {
		client_map.erase(event->_client_socket->GetSocket());
	}
}

void AcceptFunc(CMemSharePtr<CEventHandler>& event, int error) {
	client_map[event->_client_socket->GetSocket()] = std::move(event->_client_socket);
...
int main() {
	...
	std::function<void(CMemSharePtr<CEventHandler>& event, int error)> accept_func = AcceptFunc;
