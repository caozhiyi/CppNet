...
void ConnectSocket::Accept() {
    if (!_read_event) {
        _read_event = std::make_shared<Event>(ET_ACCEPT);
    }

    _read_event->AddType(ET_ACCEPT);
    _event_actions->AddAcceptEvent(_read_event);
}

void ConnectSocket::OnAccept() {
    while (true) {
        std::shared_ptr<AlloterWrap> allocter = std::make_shared<AlloterWrap>(MakePoolAlloterPtr());
        std::shared_ptr<Address> address = allocter->PoolNewSharePtr<Address>();
