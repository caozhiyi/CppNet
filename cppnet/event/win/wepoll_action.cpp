...
std::shared_ptr<EventActions> MakeEventActions() {
    return std::make_shared<WepollEventActions>();
}

WepollEventActions::WepollEventActions():
...
bool WepollEventActions::Init(uint32_t thread_num) {
    //get epoll handle. 
...
bool WepollEventActions::Dealloc() {
    Wakeup();
    return true;
}

bool WepollEventActions::AddSendEvent(Event* event) {
...
bool WepollEventActions::AddRecvEvent(Event* event) {
...
bool WepollEventActions::AddAcceptEvent(Event* event) {
...
bool WepollEventActions::AddConnection(Event* event, Address& addr) {
...
bool WepollEventActions::AddDisconnection(Event* event) {
...
bool WepollEventActions::DelEvent(Event* event) {
    auto sock = event->GetSocket();
    if (!sock) {
        return false;
    }
...
void WepollEventActions::ProcessEvent(int32_t wait_ms) {
...
void WepollEventActions::OnEvent(std::vector<epoll_event>& event_vec, int16_t num) {
...
bool WepollEventActions::AddEvent(epoll_event* ev, int32_t event_flag, uint64_t sock, bool in_actions) {
    //if not add to epoll
...
bool WepollEventActions::MakeEpollEvent(Event* event, epoll_event* &ep_event) {
    auto sock = event->GetSocket();
    if (!sock) {
        LOG_WARN("socket is already distroyed! event %s", "AddSendEvent");
        return false;
