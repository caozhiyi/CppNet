...
#ifndef __linux__
    // sync connection. 
    int16_t Connection(const std::string& ip, int16_t port, const char* buf, int32_t buf_len, const Handle& handle = 1);
#endif
    int16_t Connection(const std::string& ip, int16_t port, const Handle& handle = 1);
